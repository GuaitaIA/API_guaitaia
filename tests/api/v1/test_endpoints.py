...
class TestUserEndpoints:
    ...
    def test_get_users_as_superadmin(self, client: TestClient, admin_headers: dict):
    ...
    def test_create_user_success(self, client: TestClient, admin_headers: dict):
        ...
        with patch('app.services.user_service.UserService.create_user') as mock_create:
            ...
            response = client.post("/api/v1/users/create", data=user_data, headers=admin_headers)
            
            assert response.status_code == 200
            assert response.json()["status"] == "success"
    
    def test_delete_user_success(self, client: TestClient, admin_headers: dict):
    ...
    def test_update_password_success(self, client: TestClient, auth_headers: dict):
        ...
        with patch('app.services.user_service.UserService.update_password') as mock_update:
            ...
            response = client.patch("/api/v1/users/update/password", data=password_data, headers=auth_headers)
...
class TestDetectionEndpoints:
    ...
    def test_detect_wildfires_no_images(self, client: TestClient, auth_headers: dict):
        ...
        with patch('app.utils.dependencies.datetime') as mock_datetime:
            ...
            response = client.post("/api/v1/detection/", data=detection_data, headers=auth_headers)

        assert response.status_code == 400
        assert "Debe proporcionar al menos un conjunto de imágenes" in response.json()["detail"]

    def test_detect_wildfires_with_images_and_strings(self, client: TestClient, auth_headers: dict):
        ...
        with patch('app.utils.dependencies.datetime') as mock_datetime:
            ...
            response = client.post("/api/v1/detection/", data=detection_data, files=files, headers=auth_headers)
    ...
    def test_detect_wildfires_success(self, mock_process, client: TestClient, auth_headers: dict):
...
class TestResultsEndpoints:
    ...
    def test_get_statistics_success(self, client: TestClient, auth_headers: dict):
        ...
        with patch('app.services.results_service.ResultsService.get_statistics') as mock_stats:
            ...
            response = client.get("/api/v1/results/statistics", headers=auth_headers)
    ...
    def test_get_results_dates_success(self, client: TestClient, auth_headers: dict):
        ...
        with patch('app.services.results_service.ResultsService.get_results_dates') as mock_dates:
            ...
            response = client.get("/api/v1/results/dates", headers=auth_headers)
            
            assert response.status_code == 200
            data = response.json()
            assert len(data) == 2
    
    def test_get_results_images_success(self, client: TestClient, auth_headers: dict):
        ...
        with patch('app.services.results_service.ResultsService.get_results_images_date') as mock_images:
            ...
            response = client.get("/api/v1/results/images?date=2023-01-01", headers=auth_headers)
            
            assert response.status_code == 200
            data = response.json()
            assert len(data) == 2
    
    def test_update_image_status_success(self, client: TestClient, admin_headers: dict):
        ...
        with patch('app.services.results_service.ResultsService.update_results_images_status') as mock_update:
            ...
            response = client.put("/api/v1/results/images/status?id=1&status=true", headers=admin_headers)
